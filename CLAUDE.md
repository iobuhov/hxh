...
- **UI Library**: Mantine v9.4.1 with CSS-based theming (see ADR-002)
- **Structure**:
...
### Mantine 9 API Gotchas
Most Mantine docs and LLM training data describe **v8**. Verify prop names against the installed
source (`node_modules/@mantine/core/esm/components/{Component}/{Component}.mjs`) rather than from
memory — v9 renamed props, and unknown props are silently spread onto the DOM node instead of
erroring, so a wrong name fails at runtime with no warning.

Known renames hit so far:
- `Collapse`: v8 `in` → v9 `expanded` (see [ADR-001](widgets/nav-link/docs/decisions/ADR-001-collapse-mode.md))

Mantine 9 also runs on React 19: `Collapse` keeps closed children mounted via `<Activity mode="hidden">`,
which shows up in the DOM as `display: none !important`. That's expected, not a bug.

### Icons
Icons come from **lucide** (`lib-lucide` / `lucide-react`), not Tabler. Mantine docs examples use
`@tabler/icons-react` names like `IconChevronRight`; the lucide equivalent is `ChevronRight`.
Widgets resolve icons by kebab-case name through the `icons` map (e.g. `"chevron-right"`).
...
- Mantine version pinned at 9.4.1 via PNPM catalog (`@mantine/core`, `@mantine/hooks`)
- PostCSS processes Mantine CSS via `postcss-preset-mantine`
- Mendix CLI (`mpx`) distributed as local tarball: `pkg/mendix-mpx-0.1.2.tgz`
- Each widget has `package.xml` with Mendix metadata (bundled into `.mpk` files)
- Widget XML is generated from a per-widget YAML source (`src/{widget}.yaml`) via
  `pnpm update-xml`; edit the YAML, not the XML. Typings regenerate on `pnpm build`.
- `pnpm copy-widgets` (in `modules/finch-web`) copies built `.mpk` files into `mxproject/widgets/`.
  `mxproject/deployment/web/` is regenerated by Studio Pro on redeploy — don't hand-edit it.
